...
import argparse
import pickle
from openood.networks import vit_t
...
parser.add_argument('--arch',
                    default='resnet50',
                    # choices=['resnet50', 'swin-t', 'vit-b-16', 'regnet','other']
                    )
...
print('Postprocessor: ',postprocessor_name)
...
if args.tvs_pretrained:
    if args.arch == 'resnet50':
        ...
        preprocessor = create_transform(**cfg, is_training = False)
        net = vit_t(num_classes=1000)
        net.basemodel=model
    net.save_path=os.path.join(root, 'cache',args.arch)
