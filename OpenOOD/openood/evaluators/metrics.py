...
def compute_all_metrics(conf, label, pred):
    ...
    # use correct FPR
    auroc, aupr_in, aupr_out, fpr = auc_and_fpr_recall_corrected(conf, label, recall)
...
# auc
def fpr_at_tpr(values_in: np.ndarray, values_out: np.ndarray, tpr: float, return_t=False) -> float:
    if len(values_in) * len(values_out) == 0:
        return np.NAN
    t = np.quantile(values_in, (1 - tpr))
    fpr = (values_out >= t).mean()
    return (fpr,t) if return_t else fpr 

# auc
def auc_and_fpr_recall_corrected(conf, label, tpr_th):
    # Treat ID samples as positive (label != -1) and OOD as negative (label == -1)
    id_indicator = np.zeros_like(label)
    id_indicator[label != -1] = 1  # ID=1, OOD=0

    # Compute ROC curve with ID as positive class (no need to negate conf)
    fpr_list, tpr_list, thresholds = metrics.roc_curve(id_indicator, conf)
    # fpr = fpr_list[np.argmax(tpr_list >= tpr_th)]

    fpr=fpr_at_tpr(conf[label != -1],conf[label == -1],tpr_th)

    # PR curve for ID (positive class)
    precision_in, recall_in, thresholds_in \
        = metrics.precision_recall_curve(id_indicator, conf)

    # PR curve for OOD (positive class, requires OOD indicator and negated conf)
    ood_indicator = 1 - id_indicator  # OOD=1, ID=0
    precision_out, recall_out, thresholds_out \
        = metrics.precision_recall_curve(ood_indicator, -conf)
