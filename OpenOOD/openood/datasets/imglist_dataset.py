...
class ImglistDataset(BaseDataset):
    ...
    def getitem(self, index):
        ...
        # image_name=image_name.replace('imagenet_1k/','') # max: only if imagenet from lustre is used, remove otherwise
        ...
        if 'train' in path:
            path=path.replace('imagenet_1k/','') # max: only if imagenet train from lustre is used, remove otherwise
        sample = dict()
