...
class MDSPostprocessor(BasePostprocessor):
    ...
    def setup(self, net: nn.Module, id_loader_dict, ood_loader_dict):
        if not self.setup_flag:
            ...
            # os.makedirs(net.save_path, exist_ok=True)
            # np.save(os.path.join(net.save_path,'features.npy'),all_feats.cpu().numpy())
            # np.save(os.path.join(net.save_path,'logits.npy'),all_preds.cpu().numpy())
            # np.save(os.path.join(net.save_path,'labels.npy'),all_labels.cpu().numpy())
            # np.save(os.path.join(net.save_path,'mean.npy'),group_lasso.covariance_)
            # np.save(os.path.join(net.save_path,'cov.npy'),self.class_mean.cpu().numpy())
