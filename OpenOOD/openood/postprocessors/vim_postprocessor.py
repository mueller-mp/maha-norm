...
class VIMPostprocessor(BasePostprocessor):
    ...
    def setup(self, net: nn.Module, id_loader_dict, ood_loader_dict):
        if not self.setup_flag:
            ...
            print('self.u is finite: ',np.isfinite(self.u).all())
            ec = EmpiricalCovariance(assume_centered=True)
            ec.fit(feature_id_train - self.u)
            eig_vals, eigen_vectors = np.linalg.eig(ec.covariance_)
            self.NS = np.ascontiguousarray(
                (eigen_vectors.T[np.argsort(eig_vals * -1)[self.dim:]]).T)
            print('self.NS is finite: ',np.isfinite(self.NS).all())

            vlogit_id_train = norm(np.matmul(feature_id_train - self.u,
                                             self.NS),
                                   axis=-1)
            print('dim: ',self.dim)
            print('EV shape', eigen_vectors.shape)
            print('vlogit_id_train is finite: ',np.isfinite(vlogit_id_train).all())
            print('logit_id_train is finite: ',np.isfinite(logit_id_train).all())
            print('mean vlogit_id_train', vlogit_id_train.mean())
            self.alpha = logit_id_train.max(
                axis=-1).mean() / vlogit_id_train.mean()
            print(f'{self.alpha=:.4f}')
            print('self.alpha is finite: ',np.isfinite(self.alpha))
