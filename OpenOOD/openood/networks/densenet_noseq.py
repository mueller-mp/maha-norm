...
class _DenseLayer(nn.Module):
    ...
    def __init__(self, in_channels, bn_size, growth_rate, dropout_rate, padding):
        ...
        self.layers.add_module('batchnorm1',nn.BatchNorm2d(in_channels))
        self.layers.add_module('relu',nn.ReLU(inplace=True))
        self.layers.add_module('conv',nn.Conv2d(in_channels, bn_size * growth_rate, kernel_size=1, stride=1,
                                              bias=False))
        self.layers.add_module('batchnorm2', nn.BatchNorm2d(bn_size * growth_rate))
        self.layers.add_module('relu2', nn.ReLU(inplace=True))
        self.layers.add_module('padding', _padding[padding](1))
        self.layers.add_module('conv2',nn.Conv2d(bn_size * growth_rate, growth_rate, kernel_size=3, stride=1,
                                              bias=False))
...
class _Transition(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.layers.add_module('batchnorm1', nn.BatchNorm2d(in_channels))
        self.layers.add_module('relu', nn.ReLU(inplace=True))
        self.layers.add_module('conv', nn.Conv2d(in_channels, out_channels, kernel_size=1, stride=1, bias=False))
        self.layers.add_module('pool', nn.AvgPool2d(kernel_size=2, stride=2))

    def forward(self, input):
        return self.layers(input)


class CIFARDenseNet_nosequential(nn.Module):
    """
    ...
    def __init__(self, num_classes, init_channels, num_layers, growth_rate, dropout_rate=0, bn_size=4, reduction=2,
        ...
        super(CIFARDenseNet_nosequential, self).__init__()
        # initial conv.
    ...
    def forward(self, input,return_feature=False,return_feature_list=False):
        x = self.features(input)
        x = F.relu(self.bn1(x), inplace=True)
        x = F.adaptive_avg_pool2d(x, 1)
        features = x.view(x.size(0), -1)
        return (self.linear(features),features) if return_feature else self.linear(features)
...
def _cifar_densenet(depth, num_classes, growth_rate=12, **kwargs):
    n = (depth - 4) // 6
    model = CIFARDenseNet_nosequential(num_classes, init_channels=2 * growth_rate, num_layers=n, growth_rate=growth_rate,
                          padding="reflect", **kwargs)
    return model


def densenet100_nosequential(num_classes, **kwargs):
    return _cifar_densenet(100, num_classes, **kwargs)


def densenet40_nosequential(num_classes, **kwargs):
